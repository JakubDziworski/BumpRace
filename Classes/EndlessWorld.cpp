...
void EndlessWorld::onDisplayModeTutorial()
{
		if (isEndless())
			DialogReader::getInstance()->getTutorialDialog("bestScoreModeTut.json", G_getHud()->getCocostudioNode(), this);
		else
			DialogReader::getInstance()->getTutorialDialog("lastModeTut.json", G_getHud()->getCocostudioNode(), this);
}
